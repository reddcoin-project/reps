# REP-0005 - Reddcoin BIP32 derivation path registry
...
- **Relates:** [REP-1018](rep-1018.md), which specifies the scheme under `1018'`.
...
It does, however, follow the **spirit** of the first route. BIP43 asks that a scheme take the
number of the document defining it, so that a purpose value leads back to a specification
rather than to a constant in somebody's source tree. A Reddcoin-only scheme cannot reasonably
take a BIP number, so it does the same thing one level down: the defining REP is numbered to
match the purpose. `1018'` is specified by [REP-1018](rep-1018.md).

Self-allocated values SHOULD be assigned this way. It costs nothing, it makes the value
self-documenting, and it removes the question of where a number came from. It confers no
reservation, and §6 still applies in full.
...
| `1018'` | Release manifest signing | This REP §4.5, scheme specified by [REP-1018](rep-1018.md) | Self-allocated, §5.2 |
...
| `m/1018'/<coin>'/<index>'` | Release manifest signing key | [REP-1018](rep-1018.md) |
...
| Specified by | [REP-1018](rep-1018.md) |
| Allocation | Self-allocated, per §5.2 |
| Assigned | 2026-09-03 |
| Number chosen | To match the defining REP, per §3.1. REP-1018 specifies purpose `1018'` |
...
- [REP-1018](rep-1018.md) - the scheme under purpose `1018'`, and the first value assigned by
  the procedure in §5.
