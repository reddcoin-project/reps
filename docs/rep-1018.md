# REP-1018 - Signed release manifests for client-side verification
...
- **Uses:** BIP43 purpose `1018'`, recorded in [REP-0005](rep-0005.md). This document specifies
  the scheme; the registry records the value (§4). The REP number and the purpose value match
  deliberately, see §4.
...
The value itself is recorded in **[REP-0005](rep-0005.md)**, which holds every purpose value
Reddcoin uses and the procedure for assigning a new one. That separation is deliberate: as
ReddID, tipping, and any later scheme acquire paths of their own, allocations scattered across
the REPs that happen to need them are allocations nobody can check against.

**The number of this REP is the purpose value.** BIP43 asks that a scheme "apply for assigning
a separate BIP number and use the same number for purpose field". A Reddcoin-only scheme cannot
reasonably take a BIP number, so it does the same thing one level down: this document is
REP-1018 and the purpose is `1018'`. That does not reserve the value outside Reddcoin, and
REP-0005 §6 is explicit about the limit, but it does mean the value is traceable to a numbered
specification rather than to a constant in a script.
...
```cpp
//! The release signing key, x-only BIP340, per REP-1018. Compiled in: this is
...
- [REP-0005](rep-0005.md) - Reddcoin BIP32 derivation path registry. Records purpose `1018'`
  and the procedure that assigned it.
